...
class BeQuantOrderData(OrderData):
    def __init__(
        self,
    ...
    def init_data(self) -> BeQuantOrderData:
        ...
        if isinstance(self.order_data, dict):
            ...
            self.order_id = from_dict_get_string(data, "client_order_id", None)
            self.order_status = from_dict_get_string(data, "status", "unknown")
            self.order_side = from_dict_get_string(data, "side", "unknown")
            self.order_type = from_dict_get_string(data, "type", "unknown")
            self.order_price = from_dict_get_float(data, "price", 0.0)
            self.order_size = from_dict_get_float(data, "quantity", 0.0)
            self.executed_qty = from_dict_get_float(data, "filled_qty", 0.0)
            self.order_symbol_name = from_dict_get_string(data, "symbol", self.symbol_name)
...
class BeQuantRequestOrderData(BeQuantOrderData):
    pass


class BeQuantWssOrderData(BeQuantOrderData):
    pass


__all__ = ["BeQuantOrderData", "BeQuantRequestOrderData", "BeQuantWssOrderData"]
